...
<div class="col-xl-5 col-xl-offset-4 container">
    ...
    <form action ="{{ route('admin.courses.update', $users['id']) }}" method="post"enctype="multipart/form-data">
        @method('PUT')
        @csrf
        <input type="hidden" class="form-control" id="" name="id" value="{{ $users['id'] }}"/>
        <div class="form-group">
            <label for="name">Name:</label>
            <input type="text" class="form-control" id="" name="name" value="{{ $users['name'] }}" placeholder="Tên user"/>
            @if ($errors->has('name'))
        ...
        <div class="form-group">
            ...
            <img class="img-preview" src="{{ asset($users['email']) }}" alt="">
            @if ($errors->has('email'))
                <div class="alert alert-danger">
                    {{ $errors->first('email') }}
                </div>
        ...
        <div class="form-group">
            ...
            <input type="text" class="form-control" id="" name="phone" value="{{ $users['phone'] }}" placeholder="Số điện thoại"/>
            @if ($errors->has('phone'))
            <div class="alert alert-danger">
                {{ $errors->first('phone') }}
            </div>
